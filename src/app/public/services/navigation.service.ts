...
export class NavigationService {
  ...
  navigateToDestination() {
    ...
    if (!this.routesService.isNearby(this.destination!, routePath.path[routePath.path.length - 1])) {
  ...
  //     // Integrate extensions bidirectionally
  //     (route.extensions || []).forEach((extension: any) => {
  //       const extensionStartPoint = this.parseWaypoint(extension.startPoint);
  
  //       // Truncate the main route at the extension's start point
  ...
  //         waypoints = extendedWaypoints; // Update waypoints with extension integration
  //       }
  //     });
